  ...
  useLayoutEffect(() => {
    ...
    const mockedFetch: typeof fetch = async (input, init) => {
      ...
      if (
        ...
        if (
          currentOptions.inventoryDelayMs &&
          currentOptions.inventoryDelayMs > 0
        ) {
          ...
          return toJsonResponse(
            {
              ok: false,
              error: {
                message: "Checkout products are unavailable right now.",
              },
            },
            500,
          );
      ...
      if (pathname === "/api/checkout/confirm" && init?.method === "POST") {
        ...
        for (const line of lines) {
          const entry = inventoryRef.current.find(
        ...
        const totalAmount = lines.reduce((sum, line) => {
          const item = inventoryRef.current.find(
...
const withMockApi: Decorator = (Story, context) => {
  const options = (context.parameters.mockApi as
    | MockApiOptions
    | undefined) ?? {
