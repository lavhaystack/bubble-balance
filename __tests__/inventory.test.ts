...
describe("Inventory API Routes", () => {
  ...
  test("GET /api/inventory forwards includeArchived", async () => {
    ...
    await request(server).get("/api/inventory?includeArchived=true");
    server.close();
  ...
  test("PATCH /api/inventory/:id archives stock", async () => {
    ...
    inventoryRepository.setArchived.mockResolvedValue(archivedResult);
  ...
  test("DELETE /api/inventory/:id deletes stock", async () => {
    ...
    const response = await request(server).delete(`/api/inventory/${inventoryId}`);
    server.close();
  ...
  test("fails if environment variables are missing (from .env.test context)", async () => {
