...
describe("Suppliers API", () => {
  ...
  test("GET /api/suppliers returns list of suppliers", async () => {
  ...
  test("POST /api/suppliers fails with 400 on empty payload", async () => {
    const server = createRouteServer(POST);

    const response = await request(server)
      .post("/api/suppliers")
      .send({}); // Invalid empty object
    server.close();
  ...
  test("PATCH /api/suppliers/:id updates supplier details", async () => {
    const updatedData = { ...sampleSupplier, name: "Acme Updated" };
    supplierRepository.update.mockResolvedValue(updatedData);
    ...
    expect(response.status).toBe(200);
    expect(response.body.data.name).toBe("Acme Updated");
  });

  test("DELETE /api/suppliers/:id removes a supplier", async () => {
